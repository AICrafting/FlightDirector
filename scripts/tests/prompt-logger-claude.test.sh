...
assert_jq "ANTHROPIC_API_KEY → cost_basis actual-api" '.cost_basis=="actual-api"' "$R/prompt_log.jsonl"
...
assert_jq "unreadable transcript → null usage and cost, row still written" '.input_tokens==null and .output_tokens==null and .cost_usd==null and .cost_basis==null' "$R/prompt_log.jsonl"
...
assert_jq "unknown model keeps tokens but null cost" '.model=="mystery-model-9" and .output_tokens==500 and .cost_usd==null' "$R/prompt_log.jsonl"
...
assert_jq ".flightdirector/pricing.json override prices an otherwise unknown model" '.cost_usd != null and .cost_usd > 0' "$R/prompt_log.jsonl"
