...
#     ---
#     FlightDirector:flight@<version>[ with <Model/ver>]
...
check "signature is the last two lines: rule + FlightDirector:flight@<version>" \
	"$([ "$(last_two "$BODY")" = "$(printf -- '---\nFlightDirector:flight@%s' "$VERSION")" ] && echo 1 || echo 0)" "$BODY"
...
check "--model claude-fable-5-1 renders as 'with Fable/5.1'" \
	"$(printf '%s' "$BODY" | tail -1 | grep -qx "FlightDirector:flight@$VERSION with Fable/5.1" && echo 1 || echo 0)" "$(tail -1 <<<"$BODY")"
...
for pair in 'gpt-5.6-sol=Sol/5.6' 'claude-opus-4-7=Opus/4.7' 'claude-haiku-4-5-20251001=Haiku/4.5' 'gpt-5=GPT/5' 'anthropic/claude-sonnet-5=Sonnet/5'; do
	...
	check "model '$id' → 'with $want'" "$(tail -1 <<<"$BODY" | grep -qx "FlightDirector:flight@$VERSION with $want" && echo 1 || echo 0)" "$(tail -1 <<<"$BODY")"
done
...
check "issues update: the old version/model is replaced by the current one" \
	"$(tail -1 <<<"$BODY" | grep -qx "FlightDirector:flight@$VERSION with Fable/5.1" && echo 1 || echo 0)" "$(tail -1 <<<"$BODY")"
...
check "issues update without a model drops the old 'with' clause too" "$(tail -1 <<<"$BODY" | grep -qx "FlightDirector:flight@$VERSION" && echo 1 || echo 0)" "$(tail -1 <<<"$BODY")"
...
if [ -n "${ADF_JQ:-}" ]; then
	adf="$(printf 'para\n\n---\nFlightDirector:flight@1.0.0\n' | jq -Rs "$ADF_JQ"'md_to_adf')"
	check "jira md_to_adf turns --- into a rule node" "$(jq -e '.content[1].type == "rule"' <<<"$adf" >/dev/null && echo 1 || echo 0)" "$adf"
	check "jira md_to_adf keeps the signature line as a paragraph" "$(jq -e '.content[2].content[0].text == "FlightDirector:flight@1.0.0"' <<<"$adf" >/dev/null && echo 1 || echo 0)" "$adf"
