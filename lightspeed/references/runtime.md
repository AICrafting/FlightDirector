...
Every Lightspeed skill performs this cheap preflight before its first dispatcher call:

1. Resolve the absolute plugin root from the loaded `SKILL.md` location, then set
   `DISP=<plugin-root>/scripts/lightspeed` and `BATCH_MANIFEST=<plugin-root>/scripts/batch-manifest`.
   Do not assume the host added the plugin's `bin/` directory to `PATH`. A bare
   `lightspeed` remains acceptable when `command -v lightspeed` succeeds.
...
Examples in the skill documentation use `lightspeed` for readability. Execute them through
