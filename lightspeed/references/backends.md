...
lightspeed talks to a backend through an **adapter** — skills call verbs on the dispatcher, the
...
Each backend below gives you: a worked `.lightspeed/config.json` fragment, where to create the
token, and the **minimum** scopes/permissions the adapter actually needs — not the provider's
broadest default. Tokens always live in the gitignored `.lightspeed/secrets.json` (`code.token`
and/or `issues.token`, with `code → issues` inheritance); see
[lightspeed-setup.md](lightspeed-setup.md) for the two-file layout and axis inheritance.
...
are configured by hand-editing `.lightspeed/config.json` (and `secrets.json`) for now.
...
- [lightspeed-setup.md](lightspeed-setup.md) — the two config files, axis inheritance, `stages`, and
